      ...
      <div className="mt-12 grid gap-8 lg:grid-cols-2">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {(related && related.length ? related : industries.slice(0, 3).map((i) => i.slug)).map((relatedSlug) => {
              const r = getIndustryBySlug(relatedSlug) as Industry
              return (
                <Link key={relatedSlug} href={`/industries/${relatedSlug}`} className="px-3 py-1 rounded-full bg-white border hover:border-blue-500 hover:shadow-sm transition-all text-sm">
                  {r.icon} {r.title}
                </Link>
              )
            })}
